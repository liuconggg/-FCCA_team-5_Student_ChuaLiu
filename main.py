...
#3D Scatterplot 
reverse_gender_mapping = {v: k for k, v in encoding_mappings['gender'].items()}
reverse_stream_mapping = {v: k for k, v in encoding_mappings['stream'].items()}

# Convert gender and stream columns back to categorical values
jobDataWithSalary['gender'] = jobDataWithSalary['gender'].map(reverse_gender_mapping)
jobDataWithSalary['stream'] = jobDataWithSalary['stream'].map(reverse_stream_mapping)

# Create a 3D scatter plot with categorical values for gender and stream
fig = px.scatter_3d(data_frame=jobDataWithSalary,
                    x='gpa',
...
fig.update_layout(width=800, height=500, title='3D Scatterplot for Salary with respect to GPA & Year of Experience', title_x=0.5)
